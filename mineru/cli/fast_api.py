...
_marker_subprocess_semaphore: Optional[asyncio.Semaphore] = None
...
def create_app():
    ...
    # Marker 子进程并发控制器：从环境变量 MINERU_MARKER_MAX_CONCURRENT_PROCESSES 读取
    global _marker_subprocess_semaphore
    try:
        max_marker_procs = int(os.getenv("MINERU_MARKER_MAX_CONCURRENT_PROCESSES", "0"))
    except ValueError:
        max_marker_procs = 0

    if max_marker_procs > 0:
        _marker_subprocess_semaphore = asyncio.Semaphore(max_marker_procs)
        logger.info(f"Marker subprocess concurrency limited to {max_marker_procs}")
...
- marker: Use marker in short-lived subprocesses (supports chunking to reduce RAM peaks)
...
- vlm-http-client: Faster (client suitable for openai-compatible servers)"""
        ...
        marker_pages_per_chunk: int = Form(
            0,
            description="(Adapted only for marker backend) Split PDF into chunks of N pages. 0 uses MINERU_MARKER_PAGES_PER_CHUNK or default 50.",
        ),
        marker_subprocess_timeout_s: int = Form(
            0,
            description="(Adapted only for marker backend) Per-chunk timeout in seconds. 0 disables timeout.",
        ),
        marker_keep_chunk_artifacts: bool = Form(
            False,
            description="(Adapted only for marker backend) Keep intermediate chunk files for debugging.",
        ),
...
"""
    ...
    try:
        ...
        if backend == "marker":
            from mineru.backend.marker.chunked_convert import convert_pdf_bytes_with_marker_subprocess

            try:
                pages_per_chunk = int(
                    marker_pages_per_chunk
                    or os.getenv("MINERU_MARKER_PAGES_PER_CHUNK", "50")
                )
            except ValueError:
                pages_per_chunk = 50

            timeout_s = None
            if marker_subprocess_timeout_s and marker_subprocess_timeout_s > 0:
                timeout_s = int(marker_subprocess_timeout_s)

            marker_config: dict = {"output_format": "markdown", "use_llm": False}
            marker_config_raw = os.getenv("MINERU_MARKER_CONFIG", "").strip()
            marker_config_path = os.getenv("MINERU_MARKER_CONFIG_JSON", "").strip()
            if marker_config_raw:
                try:
                    marker_config = json.loads(marker_config_raw)
                except Exception as e:
                    logger.warning(f"Invalid MINERU_MARKER_CONFIG JSON, using defaults: {e}")
            elif marker_config_path:
                try:
                    marker_config = json.loads(Path(marker_config_path).read_text(encoding="utf-8"))
                except Exception as e:
                    logger.warning(f"Failed reading MINERU_MARKER_CONFIG_JSON, using defaults: {e}")
            marker_config["output_format"] = "markdown"
            marker_config.setdefault("use_llm", False)

            for pdf_name, pdf_bytes in zip(pdf_file_names, pdf_bytes_list, strict=True):
                parse_dir = Path(unique_dir) / pdf_name / "marker"
                await convert_pdf_bytes_with_marker_subprocess(
                    pdf_bytes=pdf_bytes,
                    pdf_name=pdf_name,
                    parse_dir=parse_dir,
                    ...
                    marker_subprocess_timeout_s=timeout_s,
                    marker_semaphore=_marker_subprocess_semaphore,
                    keep_chunk_artifacts=marker_keep_chunk_artifacts,
                )
        else:
            # 调用异步处理函数
            await aio_do_parse(
            ...
            with zipfile.ZipFile(zip_path, "w", compression=zipfile.ZIP_DEFLATED) as zf:
                for pdf_name in pdf_file_names:
                    ...
                    if backend.startswith("pipeline"):
                        ...
                        parse_dir = os.path.join(unique_dir, pdf_name, "marker")
                    else:
                    ...
                    if return_images:
                        ...
                        for ext in ("jpg", "jpeg", "png"):
                            image_paths.extend(glob.glob(os.path.join(glob.escape(images_dir), f"*.{ext}")))
            ...
            for pdf_name in pdf_file_names:
                ...
                if os.path.exists(parse_dir):
                    ...
                    if return_images:
                        ...
                        image_paths = []
                        for ext in ("jpg", "jpeg", "png"):
                            safe_pattern = os.path.join(glob.escape(images_dir), f"*.{ext}")
                            image_paths.extend(glob.glob(safe_pattern))
                        data["images"] = {
                            os.path.basename(
                                image_path
                            ): f"data:{guess_image_mime_type(image_path)};base64,{encode_image(image_path)}"
                            for image_path in image_paths
                        }
